...
# copywritingseo.pl
#
# Analogiczne do mekra.pl — dostosowane do:
# - Poczta odbiorcza: aftermarket.hosting (web1.aftermarket.hosting)
# - Poczta wysyłkowa: SES (formularz@copywritingseo.pl)
...
BUCKET_NAME="copywritingseo-attachments"
PRESIGN_FUNCTION="copywritingseo-presign"
CONTACT_FUNCTION="copywritingseo-contact"
API_NAME="copywritingseo-api"
ROLE_NAME="copywritingseo-lambda-role"
...
# ── Pobierz Hosted Zone ID automatycznie ──
echo ">>> Szukam Hosted Zone ID dla $DOMAIN..."
...
if [ -z "$HOSTED_ZONE_ID" ] || [ "$HOSTED_ZONE_ID" = "None" ]; then
  ...
  echo "    Sprawdź: aws route53 list-hosted-zones"
  exit 1
fi
...
echo "============================================"
echo "  CopywritingSEO.pl — Setup AWS"
echo "============================================"
...
aws s3api put-bucket-cors \
  --bucket "$BUCKET_NAME" \
  --cors-configuration "{\"CORSRules\":[{\"AllowedOrigins\":[\"https://copywritingseo.pl\",\"https://www.copywritingseo.pl\",\"http://localhost:4321\"],\"AllowedMethods\":[\"PUT\",\"GET\"],\"AllowedHeaders\":[\"*\"],\"MaxAgeSeconds\":3600}]}"
echo "    CORS OK"
...
POLICY_DOC="{\"Version\":\"2012-10-17\",\"Statement\":[{\"Effect\":\"Allow\",\"Action\":[\"s3:PutObject\",\"s3:GetObject\"],\"Resource\":\"arn:aws:s3:::${BUCKET_NAME}/*\"},{\"Effect\":\"Allow\",\"Action\":[\"ses:SendEmail\",\"ses:SendRawEmail\"],\"Resource\":\"*\"},{\"Effect\":\"Allow\",\"Action\":[\"logs:CreateLogGroup\",\"logs:CreateLogStream\",\"logs:PutLogEvents\"],\"Resource\":\"arn:aws:logs:*:${ACCOUNT_ID}:*\"}]}"
...
if [ -n "$EXISTING_API_ID" ] && [ "$EXISTING_API_ID" != "None" ]; then
  ...
  API_ID=$(aws apigatewayv2 create-api \
    --name "$API_NAME" \
    --protocol-type "HTTP" \
    --cors-configuration "{\"AllowOrigins\":[\"https://www.copywritingseo.pl\",\"https://copywritingseo.pl\",\"http://localhost:4321\"],\"AllowMethods\":[\"POST\",\"OPTIONS\"],\"AllowHeaders\":[\"Content-Type\"],\"MaxAge\":3600}" \
    --region "$REGION" \
...
# ============================================================
# 6. SES — weryfikacja domeny copywritingseo.pl
# ============================================================
...
# ============================================================
# 8. DNS — MX (aftermarket), SPF, DMARC, MAIL FROM
# ============================================================
echo ">>> [8/9] DNS: MX (aftermarket.hosting), SPF, DMARC, MAIL FROM"

# UWAGA: Istniejący TXT copywritingseo.pl ma google-site-verification.
# Musimy go zachować i DODAĆ SPF w tym samym rekordzie TXT.
# Route53 pozwala na wiele wartości w jednym rekordzie TXT.

# Pobierz istniejący google-site-verification
EXISTING_GSV=$(aws route53 list-resource-record-sets \
  --hosted-zone-id "$HOSTED_ZONE_ID" \
  --query "ResourceRecordSets[?Name=='${DOMAIN}.' && Type=='TXT'].ResourceRecords[].Value" \
  --output text 2>/dev/null || echo "")

echo "    Istniejace TXT: $EXISTING_GSV"

# Buduj listę wartości TXT — zachowaj google-site-verification + dodaj SPF
TXT_VALUES=""
if echo "$EXISTING_GSV" | grep -q "google-site-verification"; then
  # Zachowaj istniejący google-site-verification
  GSV_VALUE=$(echo "$EXISTING_GSV" | grep "google-site-verification" | head -1)
  # Upewnij się że jest w cudzysłowach
  if [[ "$GSV_VALUE" != \"* ]]; then
    GSV_VALUE="\"$GSV_VALUE\""
  fi
  TXT_VALUES="{\"Value\":${GSV_VALUE}},{\"Value\":\"\\\"v=spf1 include:amazonses.com include:_spf.aftermarket.pl ~all\\\"\"}"
else
  TXT_VALUES="{\"Value\":\"\\\"v=spf1 include:amazonses.com include:_spf.aftermarket.pl ~all\\\"\"}"
fi
...
aws route53 change-resource-record-sets \
  ...
  --change-batch "{\"Changes\":[
    {\"Action\":\"UPSERT\",\"ResourceRecordSet\":{\"Name\":\"${DOMAIN}\",\"Type\":\"MX\",\"TTL\":300,\"ResourceRecords\":[{\"Value\":\"10 web1.aftermarket.hosting.\"}]}},
    {\"Action\":\"UPSERT\",\"ResourceRecordSet\":{\"Name\":\"${DOMAIN}\",\"Type\":\"TXT\",\"TTL\":300,\"ResourceRecords\":[${TXT_VALUES}]}},
...
echo "    MX copywritingseo.pl → web1.aftermarket.hosting (poczta odbiorcza) ✓"
echo "    SPF copywritingseo.pl → amazonses + aftermarket ✓"
echo "    DMARC copywritingseo.pl → p=none ✓"
echo "    MAIL FROM mail.copywritingseo.pl → feedback-smtp (SES wysyłka) ✓"
...
echo "  Wysyłka SES: formularz@copywritingseo.pl → SES ($SES_REGION)"
echo ""
echo "  NASTĘPNY KROK:"
echo "  1. W komponencie Contact.astro ustaw API_URL na:"
echo "     $API_URL"
echo ""
echo "  2. Sprawdz aftermarket SPF — jesli _spf.aftermarket.pl nie istnieje,"
echo "     uzyj: v=spf1 include:amazonses.com a:web1.aftermarket.hosting ~all"
echo "     aws route53 change-resource-record-sets --hosted-zone-id $HOSTED_ZONE_ID ..."
echo ""
echo "  ⚠️  WAŻNE: Jeśli SES jest w sandbox, musisz:"
echo "  1. Zweryfikować email kontakt@copywritingseo.pl w SES"
echo "     aws ses verify-email-identity --email-address kontakt@copywritingseo.pl --region $SES_REGION"
echo "  2. LUB wyjść z sandbox (request production access)"
echo ""
echo "  DNS AFTERMARKET POCZTA:"
echo "  Serwer POP3:  web1.aftermarket.hosting"
echo "  Serwer SMTP:  web1.aftermarket.hosting"
echo "  Login:        kontakt@copywritingseo.pl"
echo "============================================"
